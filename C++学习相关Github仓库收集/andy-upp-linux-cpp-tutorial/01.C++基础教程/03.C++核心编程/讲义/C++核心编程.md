...
void func(const int &a) /* 多 const 修饰 也算 类型不同，也重载 */
{
...
{
public: 
	int m_A; 公共继承：以 public    方式 派生 的 继承类 内部 可以访问，外部也可以访问
        	 保护继承：以 protected 方式 派生 的 继承类 内部 可以访问，外部不可以访问
             私有继承：以 private   方式 派生 的 继承类 内部 可以访问，外部（有待补充）
protected:
	int m_B; 以 public    方式 派生 的 继承类 内部 可以访问，外部不可以访问
             以 protected 方式 派生 的 继承类 内部 可以访问，外部不可以访问
             以 private   方式 派生 的 继承类 内部 可以访问，外部（有待补充）
private:
	int m_C; 以 public 方式 派生 的 继承类 不可以访问
             以 protected 方式 派生 的 继承类 内部 可以访问，外部（有待补充）
             以 private   方式 派生 的 继承类 内部 不可以访问
};
